<!-- Share Modal -->
<div x-show="showShareModal" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0"
    ...
    <div class="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl w-full max-w-sm mx-4 relative border border-white/20"
        ...
        <div class="p-8">
            ...
            <div class="space-x-2 grid grid-cols-4 gap-4">
                <!-- WhatsApp -->
                <button type="button"
                    ...
                    <svg class="w-7 h-7  group-hover:scale-110 transition-transform duration-200"
                ...
                <button type="button"
                    ...
                    <svg class="w-7 h-7  group-hover:scale-110 transition-transform duration-200" fill="none"
            ...
            <div class="mt-6 pt-4 border-t border-gray-200/50">
                <p class="text-center text-xs text-gray-400">Bagikan konten dengan teman-teman Anda</p>
            </div>
        </div>
    </div>
</div>
<script>
    const productUrl = window.location.href; // Bisa diubah sesuai halaman produk
    const product = @json($product);
    const shareMessage = `Hai Sobat KomfyShare!
Mau langganan ${product.title} tanpa bikin kantong bolong? Patungan bareng di KomfyShare:

• Biaya dibagi rata, super hemat
• 100% legal & aman
• Proses cepat, langsung aktif
